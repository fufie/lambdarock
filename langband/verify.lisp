...
(defmethod ok-object? ((player player) &key context warn-on-failure)
    ...
    (verify-check (not (eq nil (get-creature-inventory player))))
    ...
    (verify-check (non-negative-integer? (get-light-radius player)))
...
(defmethod ok-object? ((obj monster-kind) &key context warn-on-failure)
  ...
  (verify-check (stringp (get-id obj)))
...
(defmethod ok-object? ((ab player-abilities) &key context warn-on-failure)
  (declare (ignore context))
  (verify-check (non-negative-integer? (get-armour-rating ab)))
  (verify-check (integerp (get-armour-modifier ab)))
  (verify-check (integerp (get-tohit-modifier ab)))
  (verify-check (integerp (get-damage-modifier ab)))
  t)
...
(defmethod ok-object? ((var-obj variant) &key context warn-on-failure)
  ...
  (verify-check (stringp (get-id var-obj)))
...
(defmethod ok-object? ((obj character-race) &key context warn-on-failure)
  ...
  ;; skip stat-changes
  ;; skip abilities
  (verify-check (non-negative-integer? (get-resists obj)))
  ;; skip classes
  ;; skip start-eq
  ;; skip skills
  t)
