...
(defmethod kill-target! ((variant variant) dungeon attacker (target player) x y)
  "Tries to remove the monster at the location."
...
(defun attack-target! (dungeon attacker target x y the-attack)
  ...
  (let ((cur-hp (current-hp target)))
    ...
    (kill-target! *variant* dungeon attacker target x y)
    ;; repaint
...
(defmethod missile-hit-creature? (attacker target missile-weapon missile)
  ...
  (let ((num (random 100)))
    ...
    (let ((bonus 0))
      ...
      (when (typep missile-weapon 'active-object)
	(incf bonus (get-tohit-modifier missile-weapon)))
      
      (when (typep missile 'active-object)
	(incf bonus (get-tohit-modifier missile)))
