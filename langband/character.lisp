...
(defun register-class& (class)
  ...
  (setf (get-char-class (get-id class)) class
...
(defun register-race& (race)
  ...
  ;; add it to table
  (check-type race character-race) 
  (setf (get-char-race (get-id race)) race
      ...
      (cond ((consp resists)
	     (dolist (i resists)
	       (cond ((and (symbolp i) (not (eq nil i)))
			  ...
			  (bit-flag-add! (get-resists race) (get-element-flag var-obj i))
			  (signal-condition 'illegal-char-race :id id
					    :desc "Illegal resist-arg for race")))
		     (t
		      (signal-condition 'illegal-char-race :id id
					:desc "Unknown resist argument for race")))))
	    (t
	     (signal-condition 'illegal-char-race :id id :desc "non-cons resist argument for char-race"))))

    (when-bind (stat-sustains (getf keyword-args :stat-sustains))
      (cond ((consp stat-sustains)
	     (dolist (i stat-sustains)
	       (if (is-legal-stat? var-obj i)
		   (bit-flag-add! (get-stat-sustains race) (stat.bit-flag (get-stat-obj var-obj i)))
		   (signal-condition 'illegal-race-data :id id
				     :desc (format nil "Not legal element in stat-sustains list: ~s" i)))
	       ))
	    (t
	     (signal-condition 'illegal-race-data :id id
			       :desc (format nil "Unknown sustain argument ~s for race ~a"
...
in the global race-table for easy access."
  ...
  (handler-case
	...
	;;    (warn "Race ~a resists ~s" (race.name race) (get-resists race))
