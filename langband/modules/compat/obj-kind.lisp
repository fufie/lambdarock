      ...
      (loop for l = (read-line in-str nil 'eof)
		...
		((#\n #\N)
		 (let ((res (split-seq-on l #\:)))
		   (when cur-obj
		     (add-new-okind! cur-obj (get-id cur-obj))
;;		     (setf (get-obj-kind (get-id cur-obj)) cur-obj)
		     (setq cur-obj nil
			   patch-val nil))
		   ...
		   (let ((num-id (parse-integer (second res))))
		     (setf (get-id cur-obj) (concatenate 'string "object-" (second res))
		...
		((#\g #\G)
		 (when cur-obj
		   ...
		   (setf (text-sym cur-obj) (text-paint-value (convert-obj (schar l 4) :colour-code)
							      (schar l 2)))
		 ...
		 (let ((res (split-seq-on l #\:)))
		   ...
		   (labels ((add-str-tval (the-symbol)
			    ...
			    (add-resist! (the-resist)
			      (error "Fix resist handling ~s" the-resist)
			      (let ((gval (ensure-gval!)))
				(pushnew the-resist (get-resists cur-obj))))
			    ...
			    (add-stat-bonus! (the-stat)
			      (let ((gval (ensure-gval!)))
				(pushnew (list the-stat '<random>) (gval.stat-modifiers gval))))
			    ...
			    (add-tunnel-bonus! (the-bonus)
			      (setf (object.tunnel cur-obj) the-bonus))
			 ...
			 (39  (add-str-tval '<light-source>)
			      ...
			      (let ((gval (ensure-gval!)))
				(setf (object.charges cur-obj) patch-val)
				(setq patch-val 0)
			      
				(case subtype-val
				  (0 (add-str-tval '<torch>)
				     (setf (get-light-radius cur-obj) 1))
				  (1 (add-str-tval '<lantern>)
				     (setf (get-light-radius cur-obj) 2))
				  (4 (add-str-tval '<phial>)
				     (setf (get-light-radius cur-obj) 3))
				  (5 (add-str-tval '<star>)
				     (setf (get-light-radius cur-obj) 3))
			 ...
			 (75  (add-str-tval '<potion>)
			      ...
			      (when (> patch-val 0)
				(let ((gval (ensure-gval!)))
				  (setf (object.food-value cur-obj) patch-val)
				  (setq patch-val 0)))
			...
			(when (> (length j) 0)
			  (cond ((string-equal j "SHOW_MODS")
				...
				((string-equal j "IGNORE_ACID")
				 (push '<acid> (get-ignores cur-obj)))
				((string-equal j "IGNORE_FIRE")
				 (push '<fire> (get-ignores cur-obj)))
				((string-equal j "IGNORE_COLD")
				 (push '<cold> (get-ignores cur-obj)))
				((string-equal j "IGNORE_ELEC")
				 (push '<electricity> (get-ignores cur-obj)))

				((string-equal j "FREE_ACT")
				 (push '<free-action> (object.abilities cur-obj)))
				((string-equal j "SLOW_DIGEST")
				 (push '<slow-digestion> (object.abilities cur-obj)))
				((string-equal j "AGGRAVATE")
				 (push '<aggravate> (object.abilities cur-obj)))
				((string-equal j "SEE_INVIS")
				 (push '<see-invisible> (object.abilities cur-obj)))
				((string-equal j "TELEPORT")
				 (push '<teleport> (object.abilities cur-obj)))
				((string-equal j "FEATHER")
				 (push '<feather-fall> (object.abilities cur-obj)))
				((string-equal j "TUNNEL")
				 (setf (object.tunnel cur-obj) patch-val))
				((string-equal j "SPEED")
				 (push '<speed> (object.abilities cur-obj)))
			       
				((string-equal j "SEARCH")
				 (push '<search> (object.skill-modifiers cur-obj)))

				((string-equal j "SLAY_UNDEAD")
				 (push '<undead> (object.slays cur-obj)))

				((string-equal j "STR")
				 (pushnew '<str> (get-stat-modifiers cur-obj)))
				((string-equal j "DEX")
				 (pushnew '<dex> (get-stat-modifiers cur-obj)))
				((string-equal j "CON")
				 (pushnew '<con> (get-stat-modifiers cur-obj)))
				((string-equal j "INT")
				 (pushnew '<int> (get-stat-modifiers cur-obj)))
				((string-equal j "WIS")
				 (pushnew '<wis> (get-stat-modifiers cur-obj)))
				((string-equal j "CHR")
				 (pushnew '<chr> (get-stat-modifiers cur-obj)))
									       
				((string-equal j "SUST_STR")
				 (push '<str> (get-stat-sustains cur-obj)))
				((string-equal j "SUST_DEX")
				 (push '<dex> (get-stat-sustains cur-obj)))
				((string-equal j "SUST_CON")
				 (push '<con> (get-stat-sustains cur-obj)))
				((string-equal j "SUST_INT")
				 (push '<int> (get-stat-sustains cur-obj)))
				((string-equal j "SUST_WIS")
				 (push '<wis> (get-stat-sustains cur-obj)))
				((string-equal j "SUST_CHR")
				 (push '<chr> (get-stat-sustains cur-obj)))
						       
				((string-equal j "RES_FIRE")
				 (bit-flag-add! (get-resists cur-obj)
						(get-element-flag *variant* '<fire>)))
		 ...
		 ((#\p #\P)
		  (when cur-obj
		    (let* ((res (split-seq-on l #\:))
		      ...
		      (setf (get-armour-rating cur-obj) (parse-integer (second res)))
		      ;; the third should be num dice and base dice
		      (let ((the-dice (parse-dice (third res))))
			(setf (get-number-of-damage-dice  cur-obj) (car the-dice)
			      (get-damage-dice cur-obj) (cdr the-dice)))
		      ;; the fourth is to-hit
		      (setf (get-tohit-modifier cur-obj) (parse-integer (fourth res)))
		      ;; the fifth is dmg
		      (setf (get-damage-modifier cur-obj) (parse-integer (fifth res)))
		      ;; the sixth is ac bonus
		      (setf (get-armour-modifier cur-obj) (parse-integer (sixth res)))
