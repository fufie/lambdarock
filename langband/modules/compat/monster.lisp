		...
		((#\N #\n)
		 (when cur-monster
		   (when coll-desc
		     ...
		     (setf coll-desc nil))
		   (add-new-mkind! cur-monster (get-id cur-monster))
;;		   (setf (get-monster-kind (get-id cur-monster)) cur-monster)
		 ...
		 (let ((res (split-seq-on l #\:)))
		   ...
		   (let ((num (parse-integer (second res))))
		     (when (> num 0)
		       ...
		       (setf (get-id cur-monster) (concatenate 'string "monster-" (second res)))
		...
		((#\G #\g)
		 ...
		 (when cur-monster
		   (let ((res (split-seq-on l #\:)))
		     ...
		     (setf (text-sym cur-monster) (text-paint-value (convert-obj (schar (caddr res) 0) :colour-code)
								    (schar (cadr res) 0)))
		     )))
		 ...
		 (when cur-monster
		   (let* ((res (split-seq-on (subseq l 2) #\|))
			 ...
			 (cond ((string-equal j "IM_ELEC")
				...
				(push '<erosion> (monster.vulnerabilities cur-monster)))
