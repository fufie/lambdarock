...
(defmethod produce-high-score-object ((variant variant) (player player))
  ...
  (flet ((to-string (arg)
    ...
    (let ((race-key (to-string (get-id (player.race player))))
	  (class-key (to-string (get-id (player.class player))))
	  (gender-key (get-id (player.gender player)))
