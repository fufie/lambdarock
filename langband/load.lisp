...
(defun %filed-variant (&key engine-num-version id variant-num-version turn information)
  ...
  (let ((var-obj *variant*)) ;; hackish
    ...
    (unless (equal (string id) (string (get-id var-obj)))
...
(defun %filed-room (&key type loc-x loc-y)
  ...
  (let ((the-kind (get-room type))
	(ret-obj nil))
    (if (or (not the-kind) (not (typep the-kind 'room-type)))
	(error "Unable to find room ~s" type)
	(let* ((new-room (funcall (room-type.constructor the-kind)
				  (room-type.id the-kind)))
	       (aobj (make-instance 'active-room :type new-room)))
...
(defun %filed-door (&key type loc-x loc-y flags)
  ...
  (let ((door (get-door *variant* type)))
    ...
    (when (integerp flags)
      ...
      (setf (is-broken? door)   (if (bit-flag-set? flags #x04) t nil))
      )
...
(defun %filed-player (&key name race class gender
  ...
  (let* ((variant *variant*)
    ...
    ;; calculated
    (setf (get-creature-inventory pl-obj) (item-table-find equipment 'eq.backpack))
...
(defmethod load-object ((variant variant) (type (eql :player)) (stream l-binary-stream))
  (let* ((str (lang.stream stream))
    ...
    ;; calculated
    (setf (get-creature-inventory pl-obj) (item-table-find (player.equipment pl-obj) 'eq.backpack))
