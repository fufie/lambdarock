  ...
  (loop named naming-loop
	...
	(unless hash-val
	  ...
	  (let ((flav (make-instance 'flavour
	    ...
	    (setf (gfx-sym flav) (tile-paint-value 0 0)
		  (text-sym flav) (text-paint-value +term-white+ #\?))
	    
	    ;; ultra-hack
	    (when (engine-allows-gfx-tiles?)
	      (setf (gfx-sym flav) (tile-paint-value 10 (+ 18 (random 4)))))
...
(defun boost-stats! (item amount)
  ...
  (let* ((okind (aobj.kind item))
	 ;;(variant *variant*)
	 (stat-table (get-stat-modifiers okind)))
    ...
    (setf (get-stat-modifiers okind) stat-table)
...
(defmethod process-world& ((variant vanilla-variant) (dungeon dungeon) (player player))
  ...
  (let ((the-turn (variant.turn variant)))
    ...
    (let ((temp-attrs (player.temp-attrs player))
	  ...
	  (dun-depth (dungeon.depth dungeon)))



    ;;(warn "Process world at ~s" the-turn)
    (cond ((= 0 dun-depth) ;; in town
    ...
    (when-bind (pls (get-light-source player))
      (unless (is-artifact? pls)
	(decf (aobj.charges pls))
	(when (< (aobj.charges pls) 1)
	  (setf (get-light-radius pls) 0))))
...
(defmethod handle-mouse-click ((variant vanilla-variant) window button x y)
  ...
  (let ((num-id (window.num-id window))
	(player *player*)
	(dungeon *dungeon*)
	(max-misc-wid (- (window.width (aref *windows* +misc-frame+)) 10))) ;; why 10?

    (unless *misc-clear*
      (setf *misc-clear* (make-array max-misc-wid :initial-element #\Space)))

    ;;(warn "~s click in ~s" button num-id) 
	   ...
	   (let* ((loc-x (int-/ x (window.tile-width window)))
	     ...
	     (cond ((eq button :right)
		   ...
		   ((eq button :left)
			...
			(output-desc (format nil "(~3d,~3d)" rx ry))
			
			(when-bind (mon (cave-monsters dungeon rx ry))
			  (when (amon.seen-by-player? (car mon))
			    (output-desc (get-creature-desc (car mon) #x08))
			    (when (is-sleeping? variant (car mon))
			      (output-string! +misc-frame+ col 0 +term-l-green+ "(asleep), ")
			      (incf col (length "(asleep), ")))
			    ))
...
(defmethod update-stuff ((variant vanilla-variant) dungeon player)
  "Updates stuff according to requested updates."
  
  (unless (any-updates? player) (return-from update-stuff nil))
  
  (let ((retval nil))

    (when (want-update? player '[mana])
      (reset-update! player '[mana])
      (calculate-creature-mana! variant player)
      (setf retval t))
    
    (when (want-update? player '[spells])
      (reset-update! player '[spells])
;;      (calculate-creature-hit-points! variant player)
      (setf retval t))

    (when (call-next-method variant dungeon player)
      (setf retval t))
...
(defun interactive-refill-item! (dungeon player)
  ...
  (let ((source (get-light-source player))
    ...
    (cond ((eq nil source)
	  ...
	  ((equal (get-id source) "lantern")
	   (setf select-fun #'(lambda (x)
				(or (equal (get-id x) "oil-flask")
				    (equal (get-id x) "lantern")))))
	  ((equal (get-id source) "torch")
	    (setf select-fun #'(lambda (x)
				 (equal (get-id x) "torch"))))
	  (t
	...
	;; now we need to get plurals right.
	(when (> (aobj.number removed-obj) 1)
	  ;;(warn "Removed obj has ~s number" (aobj.number removed-obj))
	  (let ((copy (create-aobj-from-kind removed-obj :amount 1)))
	    ;;(warn "copy[~s,~s] ~s to ~s" (aobj.number removed-obj) (aobj.number copy) (aobj.charges removed-obj) (aobj.charges copy))
	    (decf (aobj.number removed-obj))
	    ;;(warn "putting back ~s" (aobj.number removed-obj))
	    (item-table-add! the-table removed-obj) ;; put back
	    (setf removed-obj copy)))

	(let ((refill-amount 0))

	  (cond ((or (equal (get-id removed-obj) "lantern")
		     (equal (get-id removed-obj) "torch")
		     (equal (get-id removed-obj) "oil-flask"))    
		 (setf refill-amount (aobj.charges removed-obj)))
		(t
	  ...
	  (cond ((plusp refill-amount)
		 (let* ((rem-charge (aobj.charges removed-obj))
			(src-charge (aobj.charges source))
		   ...
		   (setf (aobj.charges source) src-charge
			 (aobj.charges removed-obj) rem-charge)
		   
		   (cond ((equal (get-id source) "torch")
	  ...
	  ;; neat hack
	  (when (and (equal (get-id removed-obj) "oil-flask")
		     (= (aobj.charges removed-obj) 0))
...
(defun display-monster-recall (variant player mon-know)
  "Displays recall for given monster-knowledge to *cur-win* starting from row 1."
  (declare (ignorable player))

  (let ((kind (get-monster-kind variant (get-id mon-know)))
	(num (monster.num-killed mon-know)))
    ...
    (let ((tried (monster.tried-elm mon-know))
	  (vuln (get-vulnerabilities mon-know))
	  (imm (get-immunities mon-know))
	  (res (get-resists mon-know))
	  (cur-col 2)
	  (cur-row 4)
	  (colour +term-white+))
   
      (loop for i from 0
	    for x in (variant.elements variant)
	    for bit-flag = (element.bit-flag x)
	    do
	    (cond ((bit-flag-set? vuln bit-flag)
		   (multiple-value-setq (cur-col cur-row)
		     (print-text! cur-col cur-row colour
				  (format nil "~@(~a~) is vulnerable to ~a."
					  (get-creature-desc kind #x88)
					  (element.name x)
					  ))))
		  
		  ((bit-flag-set? imm bit-flag)
		   (multiple-value-setq (cur-col cur-row)
		     (print-text! cur-col cur-row colour
				  (format nil "~@(~a~) is immune to ~a."
					  (get-creature-desc kind #x88)
					  (element.name x)
					  ))))
		  ...
		  ((bit-flag-set? res bit-flag)
		   (multiple-value-setq (cur-col cur-row)
		     (print-text! cur-col cur-row colour
				  (format nil "~@(~a~) resists ~a."
					  (get-creature-desc kind #x88)
					  (element.name x)
					  ))))
		  
		  ((bit-flag-set? tried bit-flag)
		   (multiple-value-setq (cur-col cur-row)
		     (print-text! cur-col cur-row colour
				  (format nil "~@(~a~) responds normally to ~a."
					  (element.name x)
					  (get-creature-desc kind #x88)
					  ))))
		  )))
    ...
    (when-bind (flags (monster.flags mon-know))
      (print-text! 2 6 +term-white+ (format nil "~a" flags)))
...
(defun display-object-recall (variant player obj)
  ...
  (declare (ignorable player))
  (let ((kind (get-object-kind variant (get-id obj))))
	  ...
	  (case flag
	    (<free-action>
	     (multiple-value-setq (dummy-col row)
	       (print-text! 2 row +term-white+ "Allows you to move freely")))
	    (t
	     (multiple-value-setq (dummy-col row)
	       (print-text! 2 row +term-white+ (format nil "~s" flag))))))
	))))
...
(defmethod on-move-to-coord ((variant vanilla-variant) (player player) x y)
  ...
  (when-bind (win (get-window +charinfo-frame+))
    (when (basic-frame-shown? variant)
      ...
      (let* ((row (- (window.height win) 3))
	     (time (mod (variant.turn variant) +van/turns-in-24hours+))
