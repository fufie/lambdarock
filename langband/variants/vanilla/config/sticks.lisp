...
Copyright (c) 2000-2002,2004 - Stig Erik Sandø
...
(define-object-kind "wand-light" "light"
  ...
  :on-zap (object-effect (dungeon player item)
	    (when-bind (dir (get-aim-direction))
	      ...
	      (van-fire-beam! player dir (get-spell-effect '<weak-light>)
...
(define-object-kind "wand-lightning-bolt" "lightning bolts"
  ...
  :on-zap (object-effect (dungeon player item)
	      (when-bind (dir (get-aim-direction))
		(van-fire-bolt-or-beam! player 20 dir
					(get-spell-effect '<electricity>)
					(roll-dice 6 6)
					:projected-object item)
		(possible-identify! player item)
		:still-useful))
...
(define-object-kind "wand-stone-mud" "stone to mud"
  ...
  :on-zap (object-effect (dungeon player item)
	    (when-bind (dir (get-aim-direction))
	      (van-fire-beam! player dir (get-spell-effect '<erosion>)
			      (+ 20 (randint 30)) :projected-object item
			      :flag #.(logior +project-grid+ +project-item+))
	      (possible-identify! player item)
	      :still-useful))
  :damage 1) 
...
(define-object-kind "wand-polymorph" "polymorph"
  ...
  :on-zap (object-effect (dungeon player item)
	    (when-bind (dir (get-aim-direction))
	      (van-fire-beam! player dir (get-spell-effect '<polymorph>)
			      (get-power-lvl player) :projected-object item
			      :flag #.(logior +project-grid+ +project-item+))
	      (possible-identify! player item)
	      :still-useful))
  :damage 1) 
...
(define-object-kind "wand-heal-monster" "heal monster"
  ...
  :on-zap (object-effect (dungeon player item)
	    (when-bind (dir (get-aim-direction))
	      (van-fire-bolt! player dir (get-spell-effect '<heal-monster>)
			      (roll-dice 4 6) :projected-object item)
	      (possible-identify! player item)
	      :still-useful))
  :damage 1) 
...
(define-object-kind "wand-haste-monster" "haste monster"
  ...
  :on-zap (object-effect (dungeon player item)
	    (when-bind (dir (get-aim-direction))
	      (van-fire-bolt! player dir (get-spell-effect '<haste>)
			      (get-power-lvl player) :projected-object item)
	      (possible-identify! player item)
	      :still-useful))
  :damage 1) 
...
(define-object-kind "wand-slow-monster" "slow monster"
  ...
  :on-zap (object-effect (dungeon player item)
	    (when-bind (dir (get-aim-direction))
	      (van-fire-bolt! player dir (get-spell-effect '<slow>)
			      (get-power-lvl player) :projected-object item)
	      (possible-identify! player item)
	      :still-useful))
  :damage 1) 
...
(define-object-kind "wand-sleep-monster" "sleep monster"
  ...
  :on-zap (object-effect (dungeon player item)
	    (when-bind (dir (get-aim-direction))
	      (van-fire-bolt! player dir (get-spell-effect '<sleep>)
			      (get-power-lvl player)
			      :projected-object item)
	      (possible-identify! player item)
	      :still-useful))
...
(define-object-kind "wand-destroy-door" "trap/door destruction"
  ...
  :on-zap (object-effect (dungeon player item)
	    (when-bind (dir (get-aim-direction))
	      (%simple-projection player dir
				  #.(logior +project-grid+ +project-item+
					    +project-beam+)
				  (get-spell-effect '<destroy-traps/doors>) 0
				  :projected-object item)
	      (possible-identify! player item)
	      :still-useful))
  :damage 1) 
...
(define-object-kind "wand-magic-missile" "magic missile"
  ...
  :on-zap (object-effect (dungeon player item)
	      (when-bind (dir (get-aim-direction))
		(van-fire-bolt-or-beam! player 20 dir
					(get-spell-effect '<magic-missile>)
					(roll-dice 3 4)
					:projected-object item)
		(possible-identify! player item)
		:still-useful))
...
(define-object-kind "wand-teleport-other" "teleport other"
  ...
  :on-zap (object-effect (dungeon player item)
	      (when-bind (dir (get-aim-direction))
		(van-fire-beam! player dir
				(get-spell-effect '<teleport>)
				(* 5 +max-sight+)
				:projected-object item)
		(possible-identify! player item)
		:still-useful))
  ...
  :on-zap (object-effect (dungeon player item)
	    (when-bind (dir (get-aim-direction))
	      (%simple-projection player dir
				  #.(logior +project-grid+ +project-item+
					    +project-beam+)
				  (get-spell-effect '<destroy-traps>) 0
				  :projected-object item)
	      (possible-identify! player item)
	      :still-useful))
...
(define-object-kind "wand-lightning-balls" "lightning balls"
  ...
  :on-zap (object-effect (dungeon player item)
	      (when-bind (dir (get-aim-direction))
		(van-fire-ball! player dir (get-spell-effect '<electricity>)
				64 2 :projected-object item)
		(possible-identify! player item)
		:still-useful))
  :damage 1
  :ignores '(<electricity>))
...
(define-object-kind "wand-cold-balls" "cold balls"
  ...
  :on-zap (object-effect (dungeon player item)
	      (when-bind (dir (get-aim-direction))
		(van-fire-ball! player dir (get-spell-effect '<cold>)
				96 2 :projected-object item)
		(possible-identify! player item)
		:still-useful))
  :damage 1
  :ignores '(<cold>))
...
(define-object-kind "staff-det-trap" "trap location"
  :numeric-id 300
  :text-sym (text-paint-value +term-dark+ #\_)
  :power-lvl 10
  :locations '((10 . 1))
  :weight 50
  :cost 350
...
(define-object-kind "staff-haste-monsters" "haste monsters"
  ...
  :on-zap (object-effect (dungeon player item)
	    (when (project-on-los-monsters player (get-spell-effect '<haste>)
					   (get-power-lvl player)
					   :projected-object item)
	      (possible-identify! player item))
	    :still-useful)
...
(define-object-kind "staff-slow-monsters" "slow monsters"
  ...
  :on-add-magic (magic-add (item depth status)
		    (add-charges! item (+ (randint 5) 6)))
  :on-zap (object-effect (dungeon player item)
	    (when (project-on-los-monsters player (get-spell-effect '<slow>)
					   (get-power-lvl player)
					   :projected-object item)
	      (possible-identify! player item))
	    :still-useful)
...
(define-object-kind "staff-sleep-monsters" "sleep monsters"
  ...
  :on-add-magic (magic-add (item depth status)
		    (add-charges! item (+ (randint 5) 6)))
  :on-zap (object-effect (dungeon player item)
	    (when (project-on-los-monsters player (get-spell-effect '<sleep>)
					   (get-power-lvl player)
					   :projected-object item)
	      (possible-identify! player item))
	    :still-useful)
...
(define-object-kind "staff-slowness" "slowness"
  ...
  :on-zap (object-effect (dungeon player item)
	      (when (slow-creature! player -10 (+ 15 (random 30)))
		(possible-identify! player item)
		:still-useful))
...
(define-object-kind "staff-remove-curse" "remove curse"
  ...
  :on-zap (object-effect (dungeon player item)
	    (when (plusp (remove-curse! player :light))
	      (unless (is-blind? *variant* player)
...
(define-object-kind "staff-det-evil" "detect evil"
  ...
  :on-add-magic (magic-add (item depth status)
		  (add-charges! item (+ (randint 15) 8)))
  :on-zap (object-effect (dungeon player item)
	    (when (detect-evil-monsters! dungeon player item
...
(define-object-kind "staff-dispel-evil" "dispel evil"
  ...
  :on-add-magic (magic-add (item depth status)
		    (add-charges! item (+ (randint 3) 4)))
  :on-zap (object-effect (dungeon player item)
	    (when (project-on-los-monsters player
					   (get-spell-effect '<dispel-evil>)
					   60 :projected-object item)
	      (possible-identify! player item))
	    :still-useful)
...
(define-object-kind "staff-probing" "probing"
  ...
  :on-add-magic (magic-add (item depth status)
		    (add-charges! item (+ (randint 6) 2)))
  :on-zap (object-effect (dungeon player item)
	    (when (project-on-los-monsters player (get-spell-effect '<probe>)
					   (get-power-lvl player)
					   :projected-object item)
	      (possible-identify! player item))
	    :still-useful)
...
(define-object-kind "staff-darkness" "darkness"
  ...
  :sort-value 4600
  :the-kind '<staff>
  :effect-type "darkness"
  :on-add-magic (magic-add (item depth status)
		    (add-charges! item (+ (randint 8) 8)))
  :on-zap (object-effect (dungeon player item)
	    (unless (resists-element? player '<blindness>)
	      (modify-creature-state! player '<blindness> :add (+ 3 (random 5))))
	    (when (light-area! dungeon player (location-x player)
			       (location-y player) 10 3
			       :type '<darkness> :projected-object item)
	      (possible-identify! player item))
	    :still-useful)
  :damage "1d2")
...
(define-object-kind "staff-mapping" "enlightenment"
  ...
  :the-kind '<staff>
  :on-zap (object-effect (dungeon player item)
	    (when (map-area! dungeon player item)
	      (possible-identify! player item))
	    :still-useful)
  :on-add-magic (magic-add (item depth status)
