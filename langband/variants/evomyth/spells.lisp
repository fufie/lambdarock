  ...
  (when (and (is-legal-effect-type? (clause.element clause))
	...
	(when-bind (dir (get-aim-direction))
		   (funcall (case (clause.geometry clause)
			      (linear 'evo/fire-bolt!)
			      (linear-continuous 'evo/fire-beam!)
...
(defmethod print-mana-points ((variant evomyth) player setting)
  (when (is-spellcaster? player)
    (call-next-method)))

(defmethod calculate-creature-mana! ((variant evomyth) (player player))
...
(defun evo/fire-beam! (player destination effect damage &key projected-object)
...
(defun evo/fire-bolt! (player destination effect damage &key projected-object)
  "Fires a bolt in a direction."
  (let ((flag (logior +project-kill+ +project-stop+ +project-through+)))
    (%simple-projection player destination flag effect damage :projected-object projected-object)))


(defun evo/fire-bolt-or-beam! (player beam-chance destination effect damage &key projected-object)
...
(defun evo/fire-ball! (source destination effect damage radius &key projected-object)
...
(defun evo/learn-spell! (dungeon player &key (variant *variant*))
...
(defun evo/invoke-spell! (dungeon player &key (variant *variant*))
  "Invokes a spell.. gee."
...
(defmethod apply-spell-effect! ((variant evomyth) type source target &key x y (damage 0) (state-object nil))
...
;;(defmethod apply-fire-effect! ((variant evomyth) source target &key x y (damage 0) (state-object nil))
;;  (declare (ignore x y damage source target state-object))
;;  )


(defmethod apply-spell-effect! ((variant evomyth) (type (eql '<fire>)) source (target active-object)
...
(defmethod apply-spell-effect! ((variant evomyth) (type (eql '<magic-missile>)) source (target active-monster)
			       &key
...
(defmethod apply-spell-effect! ((variant evomyth) (type (eql '<fire>)) source (target active-monster)
			       &key
			       x y (damage 0)  (state-object nil))
