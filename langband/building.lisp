...
(defun define-door-type (id name &key text-sym gfx-sym
  ...
  (let ((door-obj (make-instance 'door-type :id id :name name
    ...
    (when (integerp cave-flags-on)
      (setf (slot-value door-obj 'cave-flags-on) cave-flags-on))
    (when (integerp cave-flags-off)
      (setf (slot-value door-obj 'cave-flags-off) cave-flags-off))

    (cond ((non-negative-integer? gfx-sym)
	   (setf (gfx-sym door-obj) gfx-sym))
	  (t
	   (error "Unknown gfx-sym value ~s for door ~s" gfx-sym id)))
    
    (cond ((non-negative-integer? text-sym)
	   (setf (text-sym door-obj) text-sym))
	  (t
	   (error "Unknown text-sym value ~s for door ~s" text-sym id)))

    ;; removed effects
    (setf (gethash id table) door-obj)
...
(defmethod item-table-print ((table items-in-house)
  ...
  (declare (ignore print-selection printer-function))
